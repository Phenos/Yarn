yarn.service('storyLog', function () {
    ...
    function StoryLog() {
        ...
        this.register = function (_controller) {
            controller = _controller;
        };

        this.log = function (text) {
            controller.write(text, "log");
        };

        this.image = function (url) {
            controller.write("<img src='" + url + "' alt='coverpage'>", "image");
        };

        this.debug = function (text) {
            controller.write(text, "debug");
        };

        this.error = function (text) {
            controller.write(text, "error");
        };

        this.heading = function (text) {
            controller.write(text, "heading");
        };

        this.subHeading = function (text) {
            controller.write(text, "subHeading");
        };

        this.divider = function () {
            controller.write('<div class="divider"><svg viewBox="0 100 600 160" xmlns="http://www.w3.org/2000/svg"><g>' +
        ...
        this.thingImage = function (url) {
            controller.write('<img src="' + url + '">', "thingImage");
        };
